...
int main(int argc, char *argv[]){
        ...
        while ((memory[m] & 0xF000) != 0xF000) { // while opcode is not halt
        	if ((memory[m] & 0xF000) == 0x1000) { // if opcode is ADD
        		IR = memory[m];
        		PC = PC + 0x0001;
        		mode = IR & 0x0020;
        		if (mode == 0x0000) { // register mode
        			regIndex = (memory[m] & 0x0E00) >> 9; // store ADD here
        			srOne = (memory[m] & 0x01C0) >> 6;
        			srTwo = (memory[m] & 0x0003);
        			registers[regIndex] = registers[srOne] + registers[srTwo];
        			if ((registers[regIndex] & 0x8000) != 0x0000) {
        				CC = 'P';
        			} else if ((registers[regIndex] & 0x8000) == 0x1000){
        				CC = 'N';
        			} else {
        				CC = 'Z';
        			}
        			...
        			regIndex = (memory[m] & 0x0E00) >> 6; // store ADD here
        			registers[regIndex] = registers[srOne] + registers[srTwo];
        			if ((registers[regIndex] & 0x8000) != 0x0000) {
        				CC = 'P';
        			} else if ((registers[regIndex] & 0x8000) == 0x1000){
        	...
        	} else if ((memory[m] & 0xF000) == 0x9000) { // if opcode is NOT
        		IR = memory[m]; // 98FF
        		PC = PC + 0x0001; // increment PC so PC = 4506
        		srOne = (memory[m] & 0x01C0) >> 6;
        		regIndex = (memory[m] & 0x0E00) >> 9;
        		registers[regIndex] = !(registers[srOne]);
        		if ((registers[regIndex] & 0x8000) != 0x0000) {
        			CC = 'P';
        		} else if ((registers[regIndex] & 0x8000) == 0x1000){
        			CC = 'N';
        		...
        		if (mode == 0x0000) { // register mode
        			...
        			srOne = (memory[m] & 0x01C0) >> 6;
        			srTwo = (memory[m] & 0x001F);
        			regIndex = (memory[m] & 0x0E00) >> 6; // store AND here
